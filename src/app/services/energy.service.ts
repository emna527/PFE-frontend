...
export class EnergyService {
  ...
  getAllHistory(): Observable<EnergyReportHistory[]> {
  return this.http.get<EnergyReportHistory[]>(
    `${this.LOCAL_BASE}/history/all`
  );
}
 ...
 static getDateRange(period: 'daily' | 'weekly' | 'monthly'): { startMs: number; endMs: number } {
  ...
  const todayStart = Date.UTC(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
  const todayEnd   = Date.UTC(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999);
  ...
  switch (period) {
    ...
    case 'weekly':
      // 6 jours avant + aujourd'hui = 7 jours → HEBDOMADAIRE ✅
      return { startMs: todayStart - 6 * DAY, endMs: todayEnd };

    case 'monthly':
      // 29 jours avant + aujourd'hui = 30 jours → MENSUEL ✅
      return { startMs: todayStart - 29 * DAY, endMs: todayEnd };
  }
}
}